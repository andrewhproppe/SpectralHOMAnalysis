...
# def one_gaussian(x, a1, Ω1, σ1):
#     return a1*(σ1/(2*np.sqrt(2*np.log(2))))*np.exp(-(4*np.log(2)/(σ1**2))*(x-Ω1)**2)

one_gaussian = "w[0]/np.sqrt(2*3.1459*w[1]**2)*np.exp(-(x-w[2])**2/(2*w[1]**2))+w[3]"

two_gaussian = "  w[0]/np.sqrt(2*3.1459*w[1]**2)*np.exp(-(x-w[2])**2/(2*w[1]**2))" \
               "+ w[3]/np.sqrt(2*3.1459*w[4]**2)*np.exp(-(x-w[5])**2/(2*w[4]**2)) "

three_gaussian = "  w[0]/np.sqrt(2*3.1459*w[3]**2)*np.exp(-(x-w[6])**2/(2*w[3]**2))" \
                 "+ w[1]/np.sqrt(2*3.1459*w[4]**2)*np.exp(-(x-w[7])**2/(2*w[4]**2))" \
                 "+ w[2]/np.sqrt(2*3.1459*w[5]**2)*np.exp(-(x-w[8])**2/(2*w[5]**2))" \
                 "+ w[9]"

def make_theta_one_gaussian(params_filepath):
    ...
    var_names = ['a1', 'Ω1', 'σ1', 'y0']
    return {                     #  a1,           Ω1,           σ1            y0
        'init': np.array([p.iloc[0][0], p.iloc[0][3], p.iloc[0][6], p.iloc[0][9]]),
          'lb': np.array([p.iloc[1][0], p.iloc[1][3], p.iloc[1][6], p.iloc[1][9]]),
          'ub': np.array([p.iloc[2][0], p.iloc[2][3], p.iloc[2][6], p.iloc[2][9]]),
    }, var_names

def make_theta_two_gaussian(params_filepath):
    if params_filepath.split('.')[-1] == 'xls':
        p = pd.read_excel(params_filepath, index_col=0)
    elif params_filepath.split('.')[-1] == 'csv':
        p = pd.read_csv(params_filepath, index_col=0)
    var_names = ['a1', 'a2', 'Ω1', 'Ω2', 'σ1', 'σ2', 'y0']
    return {                     #  a1,           a2,           Ω1,           Ω2,           σ1,           σ2            y0
        'init': np.array([p.iloc[0][0], p.iloc[0][1], p.iloc[0][3], p.iloc[0][4], p.iloc[0][6], p.iloc[0][7], p.iloc[0][9]]),
          'lb': np.array([p.iloc[1][0], p.iloc[1][1], p.iloc[1][3], p.iloc[1][4], p.iloc[1][6], p.iloc[1][7], p.iloc[1][9]]),
          'ub': np.array([p.iloc[2][0], p.iloc[2][1], p.iloc[2][3], p.iloc[2][4], p.iloc[2][6], p.iloc[2][7], p.iloc[2][9]]),
    }, var_names

def make_theta_three_gaussian(params_filepath):
    if params_filepath.split('.')[-1] == 'xls':
        p = pd.read_excel(params_filepath, index_col=0)
    elif params_filepath.split('.')[-1] == 'csv':
        p = pd.read_csv(params_filepath, index_col=0)
    var_names = ['a1', 'a2', 'a3', 'Ω1', 'Ω2', 'Ω3', 'σ1', 'σ2', 'σ3', 'y0']
    return {                     #  a1,           a2,           a3,           σ1,           σ2            σ3,           Ω1,           Ω2,           Ω3,           y0
        'init': np.array([p.iloc[0][0], p.iloc[0][1], p.iloc[0][2], p.iloc[0][3], p.iloc[0][4], p.iloc[0][5], p.iloc[0][6], p.iloc[0][7], p.iloc[0][8], p.iloc[0][9]]),
          'lb': np.array([p.iloc[1][0], p.iloc[1][1], p.iloc[1][2], p.iloc[1][3], p.iloc[1][4], p.iloc[1][5], p.iloc[1][6], p.iloc[1][7], p.iloc[1][8], p.iloc[1][9]]),
          'ub': np.array([p.iloc[2][0], p.iloc[2][1], p.iloc[2][2], p.iloc[2][3], p.iloc[2][4], p.iloc[2][5], p.iloc[2][6], p.iloc[2][7], p.iloc[2][8], p.iloc[2][9]]),
    }, var_names
