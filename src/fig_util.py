...
def dress_fig(
    ticks=True, legend=True, frameon=False, tight=False, lgnd_cols=1, **kwargs
):
...
def make_subplots(width, height, nrows, ncols, dpi):
    fig, ax = plt.subplots(
        nrows=nrows, ncols=ncols, figsize=(width + 2, height + 2), dpi=dpi
    )


def densify_array(arr, density=1):
    return np.linspace(arr[0], arr[-1], int(len(arr) * density))


def make_colors(n, palette="Reds", deletion_indices=None):
    if deletion_indices is None:
        deletion_indices = []
    base_colors = np.array(sns.color_palette(palette, n))
    return np.delete(base_colors, deletion_indices, axis=0)
